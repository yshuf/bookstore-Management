...
<template>
  <div id="commonTop">
    <div
      ...
      :key="index"
      class="top-block"
      :style="hasBorder&&index!=dataArry.length-1?'border-right: 1px solid #E8E8E8;':''"
      :class="specialFun(index)"
      ref="specialDiv"
    >
      <div class="top-block-title">
        <span v-if="type=='productManage'" :class="circleFunc(index)" ref="circles"></span>
        <span v-if="type=='financingNeeds'" :class="financingCircleFunc(index)" ref="circles"></span>
        {{item.title}}
      </div>
      <div class="top-block-num">{{item.num}}{{item.unit?item.unit:''}}</div>
    </div>
  </div>
</template>
